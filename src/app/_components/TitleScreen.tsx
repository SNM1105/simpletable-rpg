...
export function TitleScreen({ onStart }: { onStart: () => void }) {
  ...
  return (
    <div className={`flex h-screen flex-col items-center justify-center bg-linear-to-br from-stone-900 via-stone-800 to-stone-900 text-foreground transition-all duration-700 ${
      isStarting ? 'opacity-0 scale-95' : 'opacity-100 scale-100'
    }`}>
      <div className={`flex flex-col items-center gap-8 px-6 text-center transition-all duration-500 ${
        ...
        <button
          onClick={handleStart}
          disabled={isStarting}
          className={`group relative overflow-hidden rounded-lg bg-blue-600 px-8 py-4 text-lg font-semibold text-white shadow-2xl shadow-blue-900/50 transition-all hover:bg-blue-700 hover:scale-105 hover:shadow-blue-900/70 disabled:cursor-not-allowed ${
